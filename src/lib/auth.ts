...
export const fetchWithAuth = async (url: string, options: RequestInit = {}) => {
  ...
  const fullUrl = isAbsoluteUrl ? url : `${process.env.NEXT_PUBLIC_API_URL || "https://evershinebackend-2.onrender.com"}${url}`
    ...
    if (refreshToken) {
      try {
        const refreshResponse = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL || "https://evershinebackend-2.onrender.com"}/api/refreshToken`,
          {
            method: "POST",
