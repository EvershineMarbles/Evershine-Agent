...
class PriceUpdateFallbackService {
  ...
  private async checkForPriceUpdates() {
    try {
      ...
      const lastCheckedParam = this.lastPriceCheck.toISOString()

      // Use the new API endpoint with lastChecked parameter
      const response = await fetch(`${apiUrl}/api/checkPriceUpdates/${this.clientId}?lastChecked=${lastCheckedParam}`, {
        headers: {
