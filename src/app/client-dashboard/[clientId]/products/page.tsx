  ...
  const fetchAgentData = useCallback(async () => {
    try {
      ...
      try {
        ...
        const clientResponse = await fetch(`${apiUrl}/api/client/agent-commission`, {
          method: "GET",
          headers: {
      ...
      console.log("Client endpoint failed, trying agent commission endpoint...")
      
      try {
        const agentResponse = await fetch(`${apiUrl}/api/agent/commission-rate`, {
          method: "GET",
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
        ...
        if (agentResponse.ok) {
          ...
          console.log("Agent data response from commission endpoint:", data)
        ...
        console.error("Error with agent endpoint:", agentError)
      }

      // Try to extract agent info from token as fallback
      try {
        ...
        if (tokenParts.length === 3) {
          ...
          if (payload.agentId && payload.agentEmail) {
            ...
            console.log(`Using token-extracted agent data with default commission rate: ${agentInfo.commissionRate}%`)
      ...
      // As a last resort, use hardcoded data with the default commission rate
      console.log("Using hardcoded agent data")
      const hardcodedAgent = {
        _id: "default-agent-id",
      ...
      // Use hardcoded data with the default commission rate as final fallback
      const hardcodedAgent = {
        _id: "default-agent-id",
        agentId: "default-agent-id",
        name: "Default Agent",
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      ...
      if (data.success && Array.isArray(data.data)) {
        ...
        console.log(
          "Original prices:",
          data.data.slice(0, 3).map((p: Product) => ({ id: p._id, price: p.price }))
        )
        ...
        console.log(
          "Adjusted prices:",
          productsWithCommission.slice(0, 3).map((p: Product) => ({
            id: p._id,
        ...
        const processedProducts = productsWithCommission.map((product: Product) => ({
          ...product,
          image:
          ...
          sampleProduct:
            processedProducts.length > 0
              ? {
                  ...
                  originalPrice: processedProducts[0].basePrice,
                  adjustedPrice: processedProducts[0].price,
                }
              : null,
        })
      } else {
