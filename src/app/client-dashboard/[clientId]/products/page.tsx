...
import { Search, Loader2, Heart, ShoppingCart, AlertCircle, Wifi, WifiOff } from "lucide-react"
import Image from "next/image"
import { useToast } from "@/components/ui/use-toast"
import { Alert, AlertDescription } from "@/components/ui/alert"
import { Badge } from "@/components/ui/badge"
import { connectToPriceUpdates, checkPriceUpdates, formatPrice, getProductId, getAuthToken } from "@/lib/price-utils"
...
export default function ProductsPage() {
  ...
  const wsConnectionRef = useRef<{ connection: WebSocket; disconnect: () => void } | null>(null)
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      try {
        ...
        // Store current client ID for other components
  ...
  useEffect(() => {
    // Check for price updates on initial load
    checkPriceUpdates().then((updated) => {
      if (updated) {
        fetchProducts()
      }
    })
    ...
    const connection = connectToPriceUpdates(
      // On price update callback
      () => {
        fetchProducts()
      },
  ...
  const toggleWishlist = useCallback(
    async (e: React.MouseEvent, product: Product) => {
      ...
      try {
        ...
        const apiUrl = process.env.NEXT_PUBLIC_API_URL || "https://evershinebackend-2.onrender.com"
        const endpoint = wishlist.includes(productId)
          ? `${apiUrl}/api/removeFromWishlist`
          : `${apiUrl}/api/addToWishlist`

        console.log(`${wishlist.includes(productId) ? "Removing from" : "Adding to"} wishlist:`, productId)
